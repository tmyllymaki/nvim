...
require('lazy').setup({
  ...
  -- Telescope is kept installed only for plugins that depend on it (orgmode).
  -- All picker functionality has been migrated to Snacks.picker.
  {
  ...
  },
  {
    ...
    dependencies = {
      ...
      { 'j-hui/fidget.nvim', enabled = false }, -- replaced by snacks.notifier
    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          -- LSP navigation keymaps (grr, gri, grd, gd, gO, gW, grt) are set
          -- globally via Snacks.picker in lua/custom/plugins/snacks.lua
  ...
  { -- Autocompletion
    ...
    opts = {
      ...
      keymap = {
        ...
        -- Custom keymaps for IDE-like behavior
        -- ['<C-space>'] = { 'show', 'show_documentation', 'hide_documentation' },
        -- ['<C-e>'] = { 'hide', 'fallback' },
        -- ['<CR>'] = { 'accept', 'fallback' }, -- Enter accepts blink.cmp completion
        ...
        --
        -- -- Tab will be handled by Copilot for suggestions, and only for snippet navigation in blink.cmp
  ...
  { -- Highlight, edit, and navigate code
    ...
    config = function(_, opts)
      ...
      -- Workaround: nvim-treesitter master branch's `set-lang-from-info-string!`
      -- directive (used for markdown code block injections) doesn't unwrap the
      -- list-of-nodes form that Neovim 0.11+ passes to directive handlers, which
      -- causes `get_node_text` to crash when opening markdown files.
